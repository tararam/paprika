...
function PrepareAndSaveToPaprika() {
	...
	try {
		...
		if(d.location.href.indexOf('cookidoo') >= 0) {
			...
			HTML = HTML
				.replace(/<html[^>]+>/,'<html>')
				.replace(/<(head|script|header|recipe-scrollspy|core-context-menu|core-tooltip|core-footer|core-modal|core-toast|core-stripe|core-tos-privacy-update|button|noscript)[\w\W]+?<\/\1>/g, '')
				.replace(/<link[^>]+>/g, '')
...
*/
				.replace(/<div class="recipe-card__picture[\w\W]+?src="([^"]+)"[\w\W]+?<\/div>/, function () {
					n.image = arguments[1];
					return '';
				})
